...
## Checking most efficient way of counting the number of step-siblings

### tl;dr

	$workCommentCt = $parent->children("template=Comment")->count();
...
public function cardShelfmark() {
	$workCommentCt = $parent->children("template=Comment")->count();	// line 44 (on 2023-05-16)
}
```
...
This caused an out-of-memory condition. I retested with only 10K iterations and multiplied by 10.
	...
	$workCommentCt = $parent->numChildren("template='Comment'");

#### 1,801µs

	$this->siblings("template='Comment'")->count;
...
#### 1,759µs
	$workCommentCt = $this->siblings("template='Comment'")->count();
