...
## Checking most efficient way of counting the number of descendants

### tl;dr

	$boxCt = wire('pages')->find("template=Box, parent=$this")->count();
...
public function cardShelfmark() {
	$boxCt = wire('pages')->find("template=Box, parent=$this")->count();	// line 45 (on 2023-05-16)
}
```
...
This caused an out-of-memory condition. I retested with only 10K iterations and multiplied by 10.

	$boxCt = $this->numDescendants("template='Box'");
...
(with $thePages defined as `wire('pages')` outside loop)

	$boxCt = $thePages->find("template=Box, parent=$this")->count();

#### 1,621µs
	$boxCt = $this->children("template=Box")->count;
