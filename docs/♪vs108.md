...
## Checking most efficient way of counting remote descendants

### tl;dr

	number_format(wire('pages')->find("has_parent=$box, template=Page")->count());
...
public function cardShelfmark() {
	number_format(wire('pages')->find("has_parent=$box, template=Page")->count());	// line 82 (on 2023-05-16)
}
```
...
#### 1,702µs
	...
	$folioCt = number_format($box->descendants("template=Page")->count());

#### 1,176µs ← *fastest*

	$folioCt = number_format(wire('pages')->find("has_parent=$box, template=Page")->count());
