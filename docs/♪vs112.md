...
## Checking most efficient way of referring to `wire` properties and functions
...
Anduin `/classes/PagePage.php`

``` php
public function cardCitation() {
		$pages = $this->wire('pages');					// line 5 (on 2023-06-16)
}
```
...
``` php
	...
	$timex += hrtime(true);
	echo tagP("", "note", number_format(($timex/1e+6)) . "ms");
```
...
#### 10ms ——— fastest!
//	I had to run this 1,000,000 times and divide by 10 to get a useable result
	$pages = $this->wire('pages');
